...
simulate_one_gen_hardy_weinberg <- function(N_aa,N_Aa, N_AA, fitnessaa, fitnessAa, fitnessAA, mut_rate) {
  ...
  p_tot <- N_aa+0.5*N_Aa
  q_tot <- N_AA+0.5*N_Aa
  pmut <- rpois(1,p_tot*mut_rate)
  qmut <- rpois(1,q_tot*mut_rate)
  p <- (p_tot+qmut-pmut)/tot_pop
  q <- (q_tot+pmut-qmut)/tot_pop
...
# Test the function
print(simulate_one_gen_hardy_weinberg(100,0,100,0.3,0.3,1,0.01))
