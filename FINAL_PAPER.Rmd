...
![](images/Screenshot%202024-11-05%20at%2013.20.36.png){width="308"}

In the above graph we noticed an interesting pattern. The rescue probability seems to rise slightly around five migrants. Although the chances of this being a meaningful pattern seemed quite small (the rescue probability at this point is only 0.5%) we decided to test this phenomenon. To do so the simulation was rerun and only the outputs for 3-7 migrants per generation were graphed. As is is made clear by the graph (and also intuition), there is no causation for the slight increase in rescue probability seen above except for stochasticity.![Proportion of populations that reached each of the 3 predetermined states. To test the pattern seen in the graph above this one depicts only populations with 3-7 mean migrants per generation](images/Screenshot%202024-11-05%20at%2013.24.19.png){width="353"}
