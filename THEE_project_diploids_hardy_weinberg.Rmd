...
```{r}
set.seed(2)
simulate_pop <- function(N_init_aa, N_init_Aa, N_init_AA, fitnessaa, fitnessAa, fitnessAA, mut_rate, t_max) {
  ...
  for (i in 1:t_max+1) {
    ...
    pop_new <- simulate_one_gen_hardy_weinberg(pop_new[1],pop_new[2], pop_new[3], fitnessaa, fitnessAa, fitnessAA, mut_rate)
    ...
    if (pop_new[1]+pop_new[2]+pop_new[3]>=5*(N_init_a+ N_init_A) | pop_new[1]+pop_new[2]==0) break
  }
...
output <- simulate_pop(1000,0,100,0.3,0.3,1,0.01,max_gen)
